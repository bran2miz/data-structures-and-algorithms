...
const toTitleCase = (arr) => {
  let upCaseArr = arr.map(word => word[0].toUpperCase()+ word.substring(1));
  return upCaseArr;
};
...
let biggerThanLuke = (arr) => {
  let newArray = arr.filter(character => character.mass > 77);
  let filteredArr = newArray.map(character => character.name);
